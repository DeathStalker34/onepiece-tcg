  ...
  const content = (
    <div className="flex flex-col items-center gap-1">
      <div className={`relative ${cardWidth} ${cardHeight}`}>
        {count === 0 ? (
          <div
            className={`${cardWidth} ${cardHeight} rounded border border-amber-900/40 bg-stone-900/20 animate-in fade-in-0 duration-200`}
          ...
          Array.from({ length: stackDepth }).map((_, i) => (
            <div
              key={i}
              className={`absolute ${cardWidth} ${cardHeight} rounded border-2 border-amber-800/80 bg-gradient-to-br from-amber-900 to-stone-900 shadow animate-in fade-in-0 duration-200`}
